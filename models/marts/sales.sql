with
    ...
    customer as (select * from {{ ref("stg_tpch__customer") }}),
    lineitem as (select * from {{ ref("stg_tpch__lineitem") }}),
    nation as (select * from {{ ref("stg_tpch__nation") }}),
    region as (select * from {{ ref("stg_tpch__region") }})
select
    r.r_name as region,
    n.n_name as pais,
...
group by r.r_name, n.n_name, o.o_orderdate, c.c_mktsegment
